...
- **커스텀 노드 미러** (`custom-nodes/dist-prebuilt/`) — 조직의 prebuilt n8n 노드를 여기에 두면
  docker 가 마운트하여 즉시 로드합니다. 이어서 setup/doctor 스킬(또는 `npm run extract-catalog`)이
  그 `.node.js` 파일들을 카탈로그(`schemas/internal-nodes.json`)로 읽어들이므로, **워크플로를 작성할 때
  oh-my-n8n 이 커스텀 노드를 인식하고 코어 노드와 동일하게 활용합니다.** (로컬에만 보관 — 절대 커밋하지 않음.)
...
- **카탈로그 기반 노드** — 생성된 워크플로의 모든 노드는 코어 노드 카탈로그
  (`schemas/n8n-core-latest.schema.json`, 502 노드) 또는 미러에서 추출된 커스텀 노드 카탈로그
  (`schemas/internal-nodes.json`)에 존재해야 합니다. author 는 커스텀 노드를 그 노드의 정확한
  `fullType` 으로 연결하며, 미등록 노드는 거부되고 하네스가 `n8n-pkg-add` / `n8n-new-node` 로 안내합니다.
